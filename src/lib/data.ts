...
async function handleFetch(url: string, options: RequestInit = {}, mockData: any) {
  if (!API_URL) {
    return mockData;
  }
...
export async function getContacts(workspaceId: number = 1): Promise<Contact[]> {
    return handleFetch(`${API_URL}/contacts?workspace_id=${workspaceId}`, {}, mockContacts);
}
...
export async function getBookingTypes(workspaceId: number = 1): Promise<BookingType[]> {
    return handleFetch(`${API_URL}/booking-types?workspace_id=${workspaceId}`, {}, mockBookingTypes);
}

export async function getAlerts(workspaceId: number = 1): Promise<Alert[]> {
    return handleFetch(`${API_URL}/alerts?workspace_id=${workspaceId}`, {}, mockAlerts);
}
...
export async function getMessages(conversationId: number): Promise<Message[]> {
    return handleFetch(`${API_URL}/messages/${conversationId}`, {}, mockMessages.filter(m => m.conversation_id === conversationId));
}
...
export async function getFormSubmissions(workspaceId: number = 1): Promise<FormSubmission[]> {
    ...
    return handleFetch(`${API_URL}/form-submissions?workspace_id=${workspaceId}`, {}, []);
}
...
export async function addContact(contact: Omit<Contact, "id" | "workspace_id">) {
  ...
  return handleFetch(`${API_URL}${endpoint}`, { method: 'POST' }, { success: true });
}
